...
import { educationalLevels } from "../../../shared/constants.js";
...
export class AppState {
  ...
  effettuaLogin(portalType: UserRole) {
    const u = this.utentiRegistrati.find(
      (u) =>
  ...
  concludiRegistrazione() {
    ...
    if (this.utentiRegistrati.some((u) => u.username === username))
      return alert("Username già registrato");
  ...
  miaCollezione() {
    return this.contenuti.filter((c) => this.haIlPossesso(c));
  }
  mieiLavori() {
